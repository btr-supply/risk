...
export const SlippageModel = () => {
  ...
  const slippageModelDescription = (
    <DescriptionCard title="Methodology" formula={String.raw`r_{vault} = \frac{b_0}{b_0 + b_1} \quad r_{target} = \text{optimal ratio} \quad \delta = f(r_{vault}, r_{target}, \text{transaction})`}>
      <Typography variant="body2" paragraph>
        <strong>Core Concept</strong>: The BTR dynamic slippage model aligns user incentives with protocol health by adjusting transaction costs based on how user actions affect the vault's token balance ratio. The system compares the current <strong>vault ratio</strong> (r_vault) with the <strong>target ratio</strong> (r_target) to determine optimal behavior.
      </Typography>
      <Typography variant="body2" paragraph>
        <strong>Ratio Definitions</strong>: The vault ratio represents the proportion of token0 in the vault (0-100%), while the target ratio represents the optimal balance for the protocol's strategy. When these ratios differ, the protocol incentivizes transactions that move the vault closer to its target through reduced slippage or even rewards.
      </Typography>
      <Typography variant="body2" paragraph>
        <strong>Incentive Alignment</strong>: Users receive <strong>lower slippage</strong> (or rewards) for transactions that improve the vault ratio, and <strong>higher slippage</strong> for transactions that worsen it. This creates natural market forces where users are economically motivated to help maintain optimal protocol balance, while arbitrageurs face increasing costs for extractive behavior.
      </Typography>
      <Typography variant="body2">
        <strong>MEV Protection</strong>: The mechanism implements optimal mechanism design principles to protect against sandwich attacks and manual arbitrage by creating endogenous cost structures that make such attacks unprofitable below optimal thresholds. See the sections below for detailed <a href="#ratiodiff0-section" style={{color: theme.colors.functional.link, textDecoration: 'underline'}}>ratioDiff0 calculation</a> and <a href="#slippage-simulation-section" style={{color: theme.colors.functional.link, textDecoration: 'underline'}}>slippage application</a>.
      </Typography>
      <FormulaLegend items={[
        { symbol: '<em>r<sub>vault</sub></em>', text: 'current vault token0 ratio (0-100%)' },
        { symbol: '<em>r<sub>target</sub></em>', text: 'target vault token0 ratio (0-100%)' },
        { symbol: '<em>b<sub>0</sub></em>', text: 'token0 balance in vault' },
        { symbol: '<em>b<sub>1</sub></em>', text: 'token1 balance in vault' },
        { symbol: '<em>δ</em>', text: 'ratioDiff0 metric (improvement measure)' },
      ]} />
    </DescriptionCard>
  );
  ...
  const ratioDiff0CurveData = generateRatioDiff0CurveDataBoth(ratioDiff0Simulation, 100);
  ...
  const ratioDiff0Description = (
    ...
    >
      <Typography variant="body2" paragraph>
        <strong>Theoretical Foundation</strong>: The BTR dynamic slippage model implements <strong>optimal mechanism design theory</strong> and <strong>MEV protection research</strong> principles. The power-function slippage creates <strong>endogenous cost structures</strong> that make sandwich attacks unprofitable while implementing <strong>dynamic pricing barriers</strong> similar to Tobin tax theory.
      </Typography>
      <Typography variant="body2" paragraph>
        <strong>Calculation Process</strong>: RatioDiff0 (δ) measures the change in vault deviation from target caused by a user transaction. The vault calculates target ratio as weighted average of liquidity ranges, compares current vs simulated post-transaction ratios, then determines the improvement/worsening: <code>ratioDiff0 = |oldRatio0 - targetRatio0| - |newRatio0 - targetRatio0|</code>.
      </Typography>
      <Typography variant="body2" paragraph>
        <strong>Incentive Alignment</strong>: Following <strong>Vickrey-Clarke-Groves (VCG) mechanism</strong> concepts, users pay their <strong>marginal impact</strong> on protocol health. Positive ratioDiff0 values indicate beneficial transactions (moving closer to target), negative values indicate worsening (moving further from target). The mechanism creates <strong>incentive compatibility</strong> where users naturally optimize protocol balance.
      </Typography>
      <Typography variant="body2" paragraph>
        <strong>Risk Zones</strong>: The chart shows critical thresholds: <strong>+10%</strong> (rewarding/beneficial), <strong>-10%</strong> (penalizing), <strong>-30%</strong> (warning zone), and <strong>-50%</strong> (danger zone). Negative zones indicate high potential slippage that will be amplified by the slippage model, creating increasing transaction costs to discourage harmful operations.
      </Typography>
      <Typography variant="body2">
        <strong>Interactive Simulation</strong>: Adjust the target ratio below to see meaningful ratioDiff0 values. Try setting target ratio different from vault ratio, then observe how different user transaction ratios affect the ratioDiff0 calculation. This feeds directly into <a href="#slippage-simulation-section" style={{color: theme.colors.functional.link, textDecoration: 'underline'}}>slippage calculation</a> where negative values result in higher slippage penalties.
      </Typography>
      <FormulaLegend items={[
        { symbol: '<em>δ</em>', text: 'ratioDiff0 in basis points' },
        { symbol: '<em>r<sub>vault</sub></em>', text: 'current vault ratio before transaction' },
        { symbol: '<em>r<sub>new</sub></em>', text: 'vault ratio after transaction' },
        { symbol: '<em>r<sub>target</sub></em>', text: 'target vault ratio' },
        { symbol: '<em>Δ<sub>0</sub></em>', text: 'user token0 amount (+ deposit, - withdrawal)' },
        { symbol: '<em>Δ<sub>1</sub></em>', text: 'user token1 amount (+ deposit, - withdrawal)' },
      ]} />
    </DescriptionCard>
  );
      ...
      controls={
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <ParameterSlider
            ...
            onChange={(v) => updateRatioDiff0Simulation({ targetRatio0: v * 100 })}
            min={0}
            max={100}
            step={1}
          ...
          <ParameterSlider
            ...
            onChange={(value) => updateRatioDiff0Simulation({ vaultBalance: value })}
            min={5000}
            max={1000000000}
          ...
          <ParameterSlider
            ...
            onChange={(v) => updateRatioDiff0Simulation({ vaultRatio0: v * 100 })}
            min={0}
            max={100}
            step={1}
            formatValue={(v) => `${v.toFixed(0)}%`}
          ...
          <ParameterSlider
            ...
            onChange={(value) => updateRatioDiff0Simulation({ userAmount: value })}
            min={100}
            max={100000000}
            formatValue={formatCurrency}
          ...
          <ParameterSlider
            ...
            onChange={(v) => updateRatioDiff0Simulation({ userRatio0: v * 100 })}
            min={0}
            max={100}
            step={1}
            formatValue={(v) => `${v.toFixed(0)}%`}
          ...
          <Box sx={{ mt: 1 }}>
            <SimulationResult
              ...
              values={[
                { key: 'deposit', label: 'Deposit', value: formatBp(currentDepositRatioDiff0) },
                { key: 'withdrawal', label: 'Withdrawal', value: formatBp(currentWithdrawalRatioDiff0) }
              ]}
              colors={{
          ...
          series={[
            {
              ...
              curve: 'monotoneX',
              showMark: ({ index }) => Math.abs(ratioDiff0CurveData[index].userRatio0 - bpToPercent(ratioDiff0Simulation.userRatio0)) < 1,
            },
            {
          ...
          series={[
            {
              ...
              curve: 'monotoneX',
              showMark: ({ index }) => Math.abs(slippageCurveData[index].ratioDiff0 - bpToPercent(simulation.slippageRatioDiff0)) < 1,
            },
          ]}
          xAxis={[{
